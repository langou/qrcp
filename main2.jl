...
if (m >= n)
   ...
   τ1 = zeros(Float64,n)
   w = zeros(Float64,1,2*n)
   jpvt1 = zeros(Int64,n)

#  geqp2_v1!( Q1, jpvt1, τ1, w )
#  geqr2!(Q1,τ1,w)
...
#       opnorm( A[:,jpvt1] - Q1 * UpperTriangular(R1), 1) / opnorm(A,1))
...
#   LAPACK.geqp3!(Q2, jpvt2, τ2 )
   ...
   @printf("|| A - Q2 * R2 ||₁ / || A ||₁ = %6.2e\n",
        opnorm( A[:,jpvt2] - Q2 * UpperTriangular(R2), 1) / opnorm(A,1))

#   issue_detected = 0;
   for i=1:n
      for j=i+1:n
         if( abs( R2[i,i] )  < norm( R2[i:j,j] ) )
            @printf("issue in pivoting :: norm( R2[%d:%d,%d] ) / abs( R2[%d,%d] ) = %6.2e\n", i, j, j, i, i, norm( R2[i:j,j] ) / abs( R2[i,i] ) );
#           issue_detected = 1;
         end
      end
   end
